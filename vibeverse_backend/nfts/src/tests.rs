...
fn get_creator_nfts_works() {
    ...
    let nft = create_nft_blueprint(creator, name.clone(), transferable);
...
fn minting_nfts_works() {
    ...
    let mut nft1 = create_nft_blueprint(creator, format!("Nft1"), false);
    let mut nft2 = create_nft_blueprint(creator, format!("Nft2"), false);
...
fn minting_limit_works() {
    ...
    let _ = do_create_nft_blueprint(creator, name, description, transferable, limit, image_url);
...
fn nft_transfer_works() {
    ...
    let mut nft = create_nft_blueprint(creator, name, transferable);
    ...
    assert_eq!(
        do_transfer_nft(creator, alice, nft.clone().id),
        Ok(())
    );
...
fn create_nft_blueprint(creator: Principal, name: String, transferable: bool) -> Nft {
    let description = format!("Description of: {}", name);
    let image_url = None;
    let limit: Option<Nat> = None;

    let id = do_create_nft_blueprint(
        creator,
        name.clone(),
        description.clone(),
        transferable.clone(),
