...
export default function Upload() {
  ...
  useEffect(() => {
    document.body.style.overflow = (modal || createCollection || createItem) ? 'hidden' : 'unset';
    return () => {
    ...
    <div className="mx-auto px-4 py-10 sm:px-6 lg:px-8 bg-gradient-to-r from-[#8360c3] to-[#2ebf91]">  
      ...
      {modal && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50 z-50">
          <Modal_Collection showModal={showModal} showCreateCollection={showCreateCollection} />
        </div>
      )}
